...
data7<-data1%>%separate(col = 1,into = c("a","b","c"),sep = "/",extra = "merge")
g<-c(1,4:8,11:15,18:22,25:29)
k<-1
numberofcalls<-vector()
for(i in g){
  subset2<-data7%>%filter(b==i)
  numberofcalls[k]<-nrow(subset2)
  k<-k+1
}
...
for(i in g){
  ...
  b<-length(a)
  c<-b+1
  a[c]<-subset2$Time[b]+0.30
  e<-diff(a,1)
  f<-length(d)
  d[[f+1]]<-e
}
call.diff<-unlist(d)
data3$call.diff<-call.diff
...
ggplot(data3, aes(x=call.diff))+geom_histogram(color="black", fill="steelblue")+facet_grid(PatientType ~ .)+xlab("")+ylab("")+labs(title =)+theme_minimal()
...
ggplot(data3, aes(x=call.diff))+geom_histogram(color="black", fill="steelblue")+facet_grid(W1 ~ .)+xlab("")+ylab("")+labs(title =)+theme_minimal()
...
ggplot(data3, aes(x=call.diff))+geom_histogram(color="black", fill="steelblue")+facet_grid(W2 ~ .)+xlab("")+ylab("")+labs(title =)+theme_minimal()
...
ggplot(data3, aes(x=call.diff))+geom_histogram(color="black", fill="steelblue")+facet_grid(W3 ~ .)+xlab("")+ylab("")+labs(title =)+theme_minimal()
